...
char TrackThruVersion[] = "TrackThru Matlab version = 8-September-2016" ;
        ...
        int nrhs, const mxArray* prhs[] )
{
          ...
          ( (nlhs == 2) || (nlhs == 3) )    )
  {
    ...
    for (i = ArgStruc.FirstBunch-1 ; i < ArgStruc.LastBunch ; i++)
    {
      ...
      gpuErrchk( cudaMalloc((void**)&TheBeam.bunches[i]->ngoodray_gpu, sizeof(int)) );
#else
      ...
      if ( (mxGetM(stop)!=1)     ||
              (mxGetN(stop)!= (unsigned int) npart) ||
              (!mxIsDouble(stop)  )    )
        goto egress ;
      ...
      gpuErrchk( cudaMemcpy(stop_local, TheBeam.bunches[i]->stop, sizeof(double)*TheBeam.bunches[i]->nray, cudaMemcpyDeviceToHost) );
#else
...
#ifdef __CUDACC__      
      gpuErrchk( cudaMemcpy(TheBeam.bunches[i]->ngoodray_gpu, &TheBeam.bunches[i]->ngoodray, sizeof(int), cudaMemcpyHostToDevice) ) ;
      free(stop_local) ;
#endif   
...
#ifdef __CUDACC__
      gpuErrchk( cudaMalloc((void**)&TheBeam.bunches[i]->ptype_gpu, sizeof(unsigned short int)*TheBeam.bunches[i]->nray) );
      gpuErrchk( cudaMemcpy(TheBeam.bunches[i]->ptype_gpu, ptype, sizeof(unsigned short int)*TheBeam.bunches[i]->nray, cudaMemcpyHostToDevice) ) ;
#endif            
    }
        ...
        int nlhs, mxArray* plhs[], int DidTracking )
{
  ...
  static const char* BPMDatFieldName[] = {
    "Index","S", "Pmod","x","y","z","P","sigma"
  };
  static const char* INSTDatFieldName[] = {
    "Index","S","x","sig11","y","sig33","sig13","z","sig55"
  } ;
  static const char* SBPMDatFieldName[] = {
    "Index","S","x","y"
  } ;
  int i,j,nBunch ;
  int is,js ;
  ...
  egress:
    ...
    if (TrackArgs->Status == 1)
    {
      ...
      for (i = TrackArgs->FirstBunch-1 ; i < TrackArgs->LastBunch ; i++)
      {
        ...
        gpuErrchk( cudaFree( TrackArgs->TheBeam->bunches[i]->ngoodray_gpu ) );
        gpuErrchk( cudaFree( TrackArgs->TheBeam->bunches[i]->ptype_gpu ) );
