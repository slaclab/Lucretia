/* This file contains the top-level source for the Matlab version of 
   mexfile RmatAtoB (the octave version will be in LucretiaOctave.c).

   Matlab usage:

		[stat,R] = RmatAtoB( start, finish ) ;

   where 

	   start, finish = the start and finish of the region of interest
		   for computing the R-matrix.
		stat = matlab cell array containing status information.
		R = 6x6 matrix containing the R-matrix from start to finish.    
		If start==finish the matrix of the single selected element 
		is returned.

   RmatAtoB can also be called with one argument, the string "version".
	In this case RmatAtoB returns its own version date, as well as the
	version dates of LucretiaCommon, LucretiaPhysics, and LucretiaMatlab.

   This file also includes RmatAtoBGetCheckArgs and RmatAtoBSetReturn, which
	process the calling args and return args, respectively.
...
char RmatAtoBVersion[] = "RmatAtoB Matlab version = 18-Apr-2005" ;

/*========================================================================*/

/* Gateway procedure for computing matrix from one point to another */
				  ...
				  int nrhs, const mxArray *prhs[] ) 
{
   ...
   char BadArgs[] = "Improper argument format for RmatAtoB" ;
	double* R=NULL ;                 /* capture pointer to RMAT array */
...
/* Check the input arguments and make sure that they are OK.  Figure
   ...
   RmatArgs = RmatAtoBGetCheckArgs( nlhs, nrhs, prhs ) ;
...
/* If the call was for a list of versions, let LucretiaMatlabVersions 
   ...
   if ( RmatArgs->Version == 1 )
	{
		plhs[0] = LucretiaMatlabVersions( RmatAtoBVersion ) ;
		return ;
	}
   ...
   else /* do some real math! */
   {
     ...
     R = (double *)RmatCalculate( RmatArgs ) ;
...
egress:
	RmatAtoBSetReturn( RmatArgs->Status, plhs, R ) ;
	return ;
...
/* Procedure RmatAtoBGetCheckArgs:  make sure that RmatAtoB was called with
...
struct RmatArgStruc* RmatAtoBGetCheckArgs( int nlhs, int nrhs, 
														 const mxArray* prhs[] ) 
{
...
/* There are two valid ways to execute RmatAtoB from the Lucretia command
   line:  
	
	  VersionCell = RmatAtoB('version') 

	will return a cell array with version information, while

		[stat,Rstruc] = RmatAtoB(first,last)

   will return a 6x6 matrix.  Obviously if there
...
/* each of the 2 arguments should be a scalar integer -- check that now */
...
/* Procedure RmatAtoBSetReturn -- packages data from a call of RmatAtoB into
   the return variable.  */

/* RET:    None.
   ABORT:  if creation of the mxArray fails.
   FAIL:   if dim(plhs) < 2; this should have been addressed in 
           RmatAtoBGetCheckArgs.  */


void RmatAtoBSetReturn( int Status, mxArray* plhs[], double* R )
{

	mxArray *ReturnMatrix ;       /* the thing we want to return */
...
/* if the R pointer is NULL, then there's no point even trying to
...
/* get a Matlab matrix mxArray for the return */

	ReturnMatrix = mxCreateDoubleMatrix( 6, 6, mxREAL ) ;
	if (ReturnMatrix == NULL)
		goto egress ;

/* get the pointer to matrix6x6's real entries */

	mxrealp = mxGetPr( ReturnMatrix ) ;
	if (mxrealp == NULL)
		goto egress ;

/* fill the matrix with values from R.  Bear in mind that 
   R stores entries row-wise (ie, R(1,2) follows R(1,1)) but
...
/* nothing dynamically allocated, so don't bother to deallocte.  However,
   if unable to construct ReturnMatrix, error abort now. */

	if (Status==0)
		mexErrMsgTxt( "Unable to construct return matrix:  RmatAtoB" ) ;
	return ;
