...
void unsplitquad(UAPNode *EleNode, mxArrayList LucEleList, UAPNode *AMLRepNode, mxArray *FLPSmx) {
 /* This routine is given the <element/> AML node, and a list of the Lucretia
  * elements it should combine into that node.  This will represent a split quad
  ...
  * field perturbations.*/
  ...
  int quadctr=0, num_poles=0;
  mxArray *PSstrucmx, *ElePSmx;
  mxArray *PSmx = mexGetVariable("global", "PS");

 /* Loop around the elements in the list to find and sum the quad fields and lengths.*/
  for (mxArrayListIter it=LucEleList.begin(); it!=LucEleList.end(); it++) {
   ...
   /* If this element is a quad, then we want to examine it.*/
    if ( !strcmp(ClassType, (char*)"QUAD") ) {
      quadctr++;
 ...
 /* Add a quadrupole node to the element, and assign the design and err nodes.*/
  ...
  if (mult_exist) {
    UAPNode *MultNode = QuadNode->addChild(ELEMENT_NODE, "multipole");
    for (int polenum=0; polenum<num_poles; polenum++) {
 ...
 /* Another loop around the contents of the list to extract orientation and aperture information.*/
  ...
  for (mxArrayListIter it=LucEleList.begin(); it!=LucEleList.end(); it++) {
   ...
   /* Only extract orientation info if this is a quad, and we haven't looked at orientation already*/
    if ( !strcmp(ClassType, (char*)"QUAD") ) {
      if (!oriented){
 ...
 /* If we found a power supply, we'd better create it!*/
  if ( hasPS ) {
    ...
    CreateAMLController(AMLRepNode, PSnum, "quadrupole:k", PSmx, namestr, BLBdoub/Ldoub, FLPSmx);
  }
