function [yfit,q,chisq_ndf] = gauss_fit2(x,y,dy,y_off)   
...
xstd=1.3*std(x(th));
p = [max(y) x_ymax xstd max(y) x_ymax xstd];

p = fminsearch(@gauss_min2,p,optimset('MaxIter',5000,'MaxFunEvals',5000),arg1);
