function [h1]=plot_magnets_external(BEAMLINE,handle,smin,smax,colorflag,names,fontsize,fontname)
%
% [h0,h1]=plot_magnets_external(BEAMLINE,handle,smin,smax,colorflag,names,fontsize,fontname);
%
% Make magnet schematic in a plot (compatible with Lucretia).
% Should be in top of other plots.
%
% INPUTs:
%
%   BEAMLINE  = Lucretia BEAMLINE structure
%   handle    = handle of the figure where to plot
%   smin      = length from which to plot
%   smax      = length up to what to plot
%   colorflag = magnets in color (1) or not (0) (OPTIONAL) [default=1]
%   names  = element names (1) or not (0) (OPTIONAL) [default=0]
...
h1=[];
if (~exist('colorflag')),colorflag=1;end
if (~exist('names')),names=0;end
if (~exist('fontname')),fontname='helvetica';end
if (~exist('fontsize')),fontsize=6;end
