...
F1 = zeros(size(u),'like',u); F2 = zeros(size(u),'like',u); 
E1 = F1;     E2 = F1;
Z1 = F1;     Z2 = F1;
Fi = F1;     Ei = F1;
Zi = F1;
lambda = zeros(1,0,'like',u); mu = zeros(1,0,'like',u); 
I = zeros(1,0,'like',u);      J  = zeros(1,0,'like',u);
...
if ~isempty(I)
    ...
    % pre-allocate space and augment if needed
        chunk = 7;
        a = zeros(chunk,mumax,'like',u);
        c = a; 
        b = a;
        a(1,:) = ones(1,mumax,'like',u);
        c(1,:) = sqrt(mu);
        b(1,:) = sqrt(1-mu);
    ...
    mmax = length(I);
        mn = double(max(n));
        phin = zeros(1,mmax,'like',u);     C  = zeros(1,mmax,'like',u);    
