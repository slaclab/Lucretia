function [h0,h1]=plot_magnets(K,S,L,P,colorflag,N,fontsize,fontname)
%
% [h0,h1]=plot_magnets(K,S,L,P,colorflag,N,fontsize,fontname);
%
% Add magnet schematic to a plot (compatible with MAD and ELEGANT).
...
if (nargin==4),colorflag=1;end
names=(nargin>5);
if (names)
  if (nargin<8),fontname='helvetica';end
  if (nargin<7),fontsize=6;end
end
...
ida=find_name('LCAV',K);        % MAD
ida=[ida;find_name('RFCW',K)];  % ELEGANT
ida=[ida;find_name('RFCA',K)];  % MAD or ELEGANT
ida=[ida;find_name('RFDF',K)];  % ELEGANT
ida=sort(ida);
if (~isempty(ida))
...
ido=find_name('OCTU',K);        % MAD or ELEGANT
if (~isempty(ido))
...
idr=find_name('SOLE',K);        % MAD or ELEGANT
if (~isempty(idr))
